...
if(isset($_GET['help']))
{
	...
	<body class="<?php echo (!$auth->isAuthorized()) ? 'hold-transition login-page' : 'hold-transition skin-blue'; ?> " style="direction:<?php echo $lang['direction']; ?>;">
...
<body class="<?php echo (!$auth->isAuthorized()) ? 'hold-transition login-page' : 'hold-transition skin-blue sidebar-mini fixed'; ?> " style="direction:<?php echo $lang['direction']; ?>;">
<div class="wrapper">
...
if($auth->isAuthorized()){
	?>
     <header class="main-header">
      ...
      <span class="logo-mini"><b>b</b>LA</span>
      <!-- logo for regular state and mobile devices -->
      <span class="logo-lg"><b>bootstrap</b>LiteAdmin</span>
       </a>
  ...
  <a href="#" class="sidebar-toggle" data-toggle="offcanvas" role="button">
      ...
      <li><a id="autoComplete" >Toggle AutoComplete</a></li>
      </ul>
</nav>
     </header>
...
if(!$auth->isAuthorized())
{
	...
	echo "<div class='login-logo'><a href='".$_SERVER["PHP_SELF"]."'><b>".PROJECT." </b>v".VERSION."</a></div>";
	...
	echo "<input type='password' class='form-control' placeholder='".$lang['passwd']."' name='password'/><span class='glyphicon glyphicon-lock form-control-feedback'></span>
</div";
	echo "    <div class='row'><div class='col-xs-8'><div class='checkbox icheck'><label><input type='checkbox' name='remember' value='yes' checked='checked'/> ".$lang['remember']."</label></div></div></div>";
	echo " <div class='col-xs-4'><button type='submit' class='btn btn-primary btn-block btn-flat'>".$lang['login']."</button></div></div>";
	echo "<input type='hidden' name='login' value='true' />";
	echo "</form>";
	echo "</div>";
	echo "</div>";
	echo "<br/>";
	echo "<p class='login-box-msg'>".$lang['powered']." <a href='".PROJECT_URL."' target='_blank' style='font-size:11px;'>".PROJECT."</a> |"; 
...
else // the database array is empty, offer to create a new database
{
	...
	if($directory!==false && is_writable($directory))
	{
		...
		echo '<h3 class="box-title">No Databases Found</h3>';
		echo '</div>';
		...
		echo $token_html;
				echo "<div class='row'><div class='col-xs-4'><div class='form-group has-feedback'><input type='text' name='new_dbname' placeholder='Database Name...' class='form-control input-sm'/><span class='glyphicon glyphicon-hdd form-control-feedback'></span></div></div>";
		...
		echo "<button type='submit' class='btn btn-primary btn-block btn-flat'>".$lang['create']."</button></div>";
	;
		echo "</form>";
...
<div class="pull-left image">
        <span class='glyphicon glyphicon-hdd ' style="color: white;"></span>
      </div>
      <?php
echo "<div class='pull-left info'>
        <p>".PROJECT."</p>

        <a href='".PAGE."'><i class='fa fa-circle text-success'></i> v".VERSION." - </a>
      </div>";
echo '</div>';
echo "<ul class='sidebar-menu' >";
echo '<li class="treeview">
        <a href="#"><span>Help Links</span> <i class="fa fa-angle-left pull-right"></i></a>';
echo '<ul class="treeview-menu">';
echo "<li><a href='javascript:void' onclick='openHelp(\"top\");'>".$lang['docu']."</a></li>";
echo "<li><a href='http://www.gnu.org/licenses/gpl.html' target='_blank'>".$lang['license']."</a></li>";
echo "<li><a href='".PROJECT_URL."' target='_blank'>".$lang['proj_site']."</a></li>";
echo "</ul>";
echo "</ul>";
echo "</aside>";
...
echo "<li class=".((!$target_table) ? 'active' : ' ')."><a href='".PAGE."'><i class='fa fa-dashboard'></i>".htmlencode($currentDB['name'])."</a></li>";
if($target_table){
echo "<li class='".(($target_table) ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=row_view'>".htmlencode($target_table)."</a></li>";
}
echo "</ol>";

echo "<div class='row'><div class='col-md-3 col-sm-6 col-xs-12'><div class='info-box'><span class='info-box-icon bg-red'><i class='fa fa-database'></i></span>
...
for($i=0; $i<sizeof($result); $i++)
{
	...
	{
		echo "<span class='info-box-text'>[".$lang[$result[$i]['type']=='table'?'tbl':'view']."]";
		echo "<a href='?action=row_view&amp;table=".urlencode($result[$i]['name'])."'";
		if ($target_table == $result[$i]['name'])
			echo " ";
		echo ">".htmlencode($result[$i]['name'])."</a></span> ";
		$j++;
	}
}
if($j==0)
	echo "<span class='info-box-number'>".$lang['no_tbl']."</span>";
echo "</div>";
echo "</div></div><div class='col-xs-4'></div><div class='col-xs-4'></div></div></section><section class='content'>";
if($directory!==false && is_writable($directory))
{
	echo "<div class='row'>";
	echo "<div class='col'>";
	if(!target_table){
	$view = "structure";
echo '<section class="col-lg-12 connectedSortable"><div class="nav-tabs-custom"><ul class="nav nav-tabs pull-right">';
...
if(!$target_table && !isset($_GET['confirm']) && (!isset($_GET['action']) || (isset($_GET['action']) && $_GET['action']!="table_create"))) //the absence of these fields means we are viewing the database homepage
{
	...
	echo "<li class='".(($view=="structure") ? 'active' : ' ')."'><a  href='?view=structure'>".$lang['struct']."</a></li>";
	echo "<li class='".(($view=="sql") ? 'active' : ' ')."'><a  href='?view=sql'>".$lang['sql']."</a></li>";
	echo "<li class='".(($view=="export") ? "active" : ' ')."'><a  href='?view=export'>".$lang['export']."</a></li>";
    echo "<li class='".(($view=="import") ? "active" : ' ')."'><a  href='?view=import'>".$lang['import']."</a></li>";
	echo "<li class='".(($view=="vacuum") ? "active" : ' ')."'><a  href='?view=vacuum'>".$lang['vac']."</a></li>";
	if($directory!==false && is_writable($directory))
	{
			echo "<li class='".(($view=="rename") ? "active" : ' ')."'><a  href='?view=rename'>".$lang['db_rename']."</a></li>";
			echo "<li class='".(($view=="delete") ? "active" : ' ')." delete_db'><a  href='?view=delete'><span>".$lang['db_del']."</span></a></li>";
	}
  ...
  //- Switch on $view (actually a series of if-else)
    switch($view){
    	...
    	if(isset($dbexists))
		{
            ...
            echo "<h3 class='box-title'>".$lang['err']."</h3>";
            echo "</div>";
            echo "<div class='box-body'>";
			sprintf($lang['db_exists'], htmlencode($dbname));
			echo "</div>";
			echo "</div>";
		}
		if($db->isWritable() && !$db->isDirWritable())
		{
			echo "<div class='box box-warning'>";
            echo "<div class='box-header'>";
            echo "<h3 class='box-title'>".$lang['attention']."</h3>";
            echo "</div>";
            echo "<div class='box-body'>";
			echo $lang['directory_not_writable'];
			echo "</div>";
			echo "</div>";
		}
		if(isset($extension_not_allowed))
		{
				echo "<div class='box box-warning'>";
            echo "<div class='box-header'>";
            echo "<h3 class='box-title'>".$lang['extension_not_allowed']."</h3>";
            echo "</div>";
            echo "<div class='box-body'>";
			echo implode(', ', array_map('htmlencode', $allowed_extensions));
			echo $lang['add_allowed_extension'];
			echo "</div>";
			echo "</div>";
		}
			...
			if ($auth->isPasswordDefault())
		{
				echo "<div class='box box-warning'>";
            echo "<div class='box-header'>";
            echo "<h3 class='box-title'>".$lang['warn0']."</h3>";
            echo "</div>";
            echo "<div class='box-body '>";
			echo "<p>".sprintf($lang['warn_passwd'],(is_readable('phpliteadmin.config.php')?'phpliteadmin.config.php':PAGE))."</p>";			
			echo "<p>".$lang['add_allowed_extension']."</p>";
			echo "</div>";
			echo "</div>";
		}
		echo '<div class="box box-default collapsed-box">';
  		echo '<div class="box-header with-border">';
        echo '<h3 class="box-title">System Stats</h3>';
        echo '<div class="box-tools pull-right">';
		echo " <a href='".PROJECT_URL."' target='_blank' id='oldVersion' style='display: none;' class='btn btn-box-tool'>".$lang['new_version']."</a>";
		echo '<button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-plus"></i></button>';
...
echo "<div class='row'>";
			...
			echo "<div class='box'>";
			echo "<div class='box-header'>";
			echo "<h3 class='box-title'>Current DB Items</h3>";
			echo "</div>";
...
if(!isset($_GET['confirm']) && $target_table && isset($_GET['action']) && ($_GET['action']=="table_export" || $_GET['action']=="table_import" || $_GET['action']=="table_sql" || $_GET['action']=="row_view" || $_GET['action']=="row_create" || $_GET['action']=="column_view" || $_GET['action']=="table_rename" || $_GET['action']=="table_search" || $_GET['action']=="table_triggers"))
{
	...
	if($target_table_type == 'table')
	{
		...
		echo "<li ".(($_GET['action']=="row_view") ? 'class="active"' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=row_view'>".$lang['browse']."</a></li>";
		echo "<li class='".(($_GET['action']=="column_view") ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=column_view'>".$lang['struct']."</a></li>";
		echo "<li class='".(($_GET['action']=="table_sql") ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=table_sql'>".$lang['sql']."</a></li>";
		echo "<li class='".(($_GET['action']=="table_search") ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=table_search'>".$lang['srch']."</a></li>";
		echo "<li class='".(($_GET['action']=="row_create") ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=row_create'>".$lang['insert']."</a></li>";
		echo "<li class='".(($_GET['action']=="table_export") ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=table_export'>".$lang['export']."</a></li>";
		echo "<li class='".(($_GET['action']=="table_import") ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=table_import'>".$lang['import']."</a>";
		echo "<li class='".(($_GET['action']=="table_rename") ? 'active' : ' ')."'><a href='?table=".urlencode($target_table)."&amp;action=table_rename'>".$lang['rename']."</a>";
		echo "<li class='".(($_GET['action']=="table_empty") ? 'active' : ' ')." empty'><a href='?action=table_empty&amp;table=".urlencode($target_table)."'>".$lang['empty']."</a>";
		echo "<li class='".(($_GET['action']=="table_drop") ? 'active' : ' ')."'><a href='?action=table_drop&amp;table=".urlencode($target_table)."'>".$lang['drop']."</a>";
	}
	else
	//- HTML: tabs for views
	{
...
if(isset($_GET['action']) && !isset($_GET['confirm']))
{
	...
	//- Table actions
		...
		echo "<div class='tab-pane active'>";
			...
			echo "<h3 class='box-title'>".$lang['create_tbl'].": '".htmlencode($_POST['tablename'])."'</h3>";
			echo "</div>";
		...
		case "table_search":
			...
			if(isset($_GET['done']))
			{
				...
				if(sizeof($arr)>0)
				{
					...
					for($j=0; $j<sizeof($arr); $j++)
					{
						...
						if($target_table_type == 'table')
						{
							echo $tdWithClass."<a class='btn btn-primary edit' href='?table=".urlencode($target_table)."&amp;action=row_editordelete&amp;pk=".urlencode($pk)."&amp;type=edit' title='".$lang['edit']."'><span>".$lang['edit']."</span></a><a href='?table=".urlencode($target_table)."&amp;action=row_editordelete&amp;pk=".urlencode($pk)."&amp;type=delete' title='".$lang['del']."' class='btn btn-danger delete'><span>".$lang['del']."</span></a></td>";
						}
			...
			else
			{
				...
				echo "</tbody>";
				echo "</table>";
				echo "<button type='submit'  class='btn btn-primary'>".$lang['srch']."</button>";
	...
	//- Row actions
		...
		echo "<div class='tab-pane active'>";
			...
			echo "<form action='?action=row_view&amp;table=".urlencode($target_table)."' method='post'>";
			echo $token_html;
			echo "<button type='submit'  name='show' class='btn btn-primary'/=>".$lang['show']."</button> ";
			echo "<input type='text' name='numRows' value='".$_SESSION[COOKIENAME.'numRows']."'/> ";
			echo $lang['rows_records'];

			if(intval($_POST['startRow']+$_SESSION[COOKIENAME.'numRows']) < $rowCount)
				echo "<input type='text' name='startRow'  value='".intval($_POST['startRow']+$_SESSION[COOKIENAME.'numRows'])."'/>";
			else
				echo "<input type='text' name='startRow'  value='0'/> ";
			echo $lang['as_a'];
			echo " <select name='viewtype' class='select2' style='width:87.5px;' data-placeholder='Select a Display Type'>";
			...
			if($resultRows>0)
			{
				...
				if(!isset($_SESSION[COOKIENAME.'viewtype']) || $_SESSION[COOKIENAME.'viewtype']=="table")
				{
					...
					echo $token_html;
					echo "<table id='dataTable_table' class='table table-bordered table-hover table-striped'>";
					echo "<thead>";
					echo "<tr>";
					if($target_table_type == 'table')
					{
					...
					for($i=0; $row = $db->fetch($table_result); $i++)
					{
						...
						{
							echo $tdWithClass;
							echo "<input type='checkbox' name='check[]' value='".htmlencode($pk)."' class='checkbox select_row iCheck' id='check_".htmlencode($i)."'/>";
							echo "</td>";
							echo "<td>";
							// -g-> Here, we need to put the PK in as the link for both the edit and delete.
							echo "<a class='btn btn-primary' href='?table=".urlencode($target_table)."&amp;action=row_editordelete&amp;pk=".urlencode($pk)."&amp;type=edit' title='".$lang['edit']."' >".$lang['edit']."</a>";
							echo "<a class='btn btn-danger' href='?table=".urlencode($target_table)."&amp;action=row_editordelete&amp;pk=".urlencode($pk)."&amp;type=delete' title='".$lang['del']."' >".$lang['del']."</a>";
							echo "</td>";
						}
						for($j=0; $j<sizeof($result); $j++)
						{
					...
					if($target_table_type == 'table')
					{
						echo "<a  class='checkAll btn btn-primary'>".$lang['chk_all']."</a> / <a  class='unCheckAll btn btn-primary'>".$lang['unchk_all']."</a> <i>".$lang['with_sel'].":</i> ";
	...
	//- Column actions
		...
		case "column_view":
			...
			echo "<th>".$lang['col']." #</td>";
			echo "<th>".$lang['fld']."</td>";
			echo "<th>".$lang['type']."</td>";
			echo "<th>".$lang['not_null']."</td>";
			echo "<th>".$lang['def_val']."</td>";
			echo "<th>".$lang['prim_key']."</td>";
			echo "</tr>";
			echo "</thead>";
            echo "<tbody>";
			$noPrimaryKey = true;
			...
			for($i=0; $i<sizeof($result); $i++)
			{
				...
				{
					echo $tdWithClass;
					echo "<input type='checkbox' class='select_row' name='check[]' value='".htmlencode($fieldVal)."' id='check_".$i."'/>";
					echo "</td>";
					echo $tdWithClass;
					echo "<a href='?table=".urlencode($target_table)."&amp;action=column_edit&amp;pk=".urlencode($fieldVal)."' title='".$lang['edit']."' class='btn btn-primary edit'><span>".$lang['edit']."</span></a>";
					echo "</td>";
					echo $tdWithClass;
					echo "<a href='?table=".urlencode($target_table)."&amp;action=column_confirm&amp;action2=column_delete&amp;pk=".urlencode($fieldVal)."' title='".$lang['del']."' class='btn btn-danger delete'><span>".$lang['del']."</span></a>";
					echo "</td>";
				}
				echo $tdWithClass;
...
echo "</tbody>";
			...
			{
				echo "<a class='checkAll btn btn-primary'>".$lang['chk_all']."</a> / <a class='unCheckAll btn btn-primary'>".$lang['unchk_all']."</a> <i>".$lang['with_sel'].":</i> ";
				echo "<select class='select2' style='width:95.5px;' name='action2'>";
				//echo "<option value='edit'>".$lang['edit']."</option>";
			...
			if($target_table_type == 'table')
			{
				...
				echo $token_html;
				echo "<input type='hidden' name='tablename' value='".htmlencode($target_table)."'/>";
				echo $lang['add']." <input type='text' name='tablefields' style='width:30px;' value='1'/> ".$lang['tbl_end']." <button type='submit' name='addfields' class='btn btn-primary'>".$lang['go']."</button>";
				echo "</form>";
			}
			...
			echo "<h3 class='box-title'>".$lang['query_used_'.$target_table_type]."</h3>";
			echo "</div>";
			echo "<div class='box-body'>";
			echo "<span>".htmlencode($master[0]['sql'])."</span>";
			echo "</div>";
			echo "</div>";
			...
			if($target_table_type != 'view')
			{
				...
				echo $lang['create_index2']." <input type='text' name='numcolumns' style='width:30px;' value='1'/> ".$lang['cols']." <button type='submit'  name='addindex' class='btn btn-primary'>".$lang['go']."</button>";
...
echo "</div>";
		...
		echo "<div class='box-header'>";
			echo "<h3 class='box-title'>".sprintf($lang['new_fld'],htmlencode($_POST['tablename']))."</h3>";
echo "<div class='box-tools'>";
				echo "<a class='btn btn-box-tool' href='?table=".urlencode($_POST['tablename'])."&amp;action=column_view'><i class='fa fa-close'></i></a>";
				echo "</div>";
				echo "</div>";
			if($_POST['tablefields']=="" || intval($_POST['tablefields'])<=0)
				echo $lang['specify_fields'];
		...
		case "column_confirm":
			...
			else
			{
				...
				echo "</h3>";
				echo '<div class="box-tools">';
				echo "<a class='btn-box-tool' href='?table=".urlencode($target_table)."&amp;action=column_view'>".$lang['cancel']."</a>";
				echo "</div>";
				echo "</div>";
				echo "<div class='box-body'>";
				...
				echo "<button type='submit' class='btn btn-danger'>".$lang['confirm']."</button>";
				echo "</form>";
				echo "</div>";
				echo "</div>";
			}
...
echo "<div class='row'>";
	echo "<div class='col'>";
	echo "<div class='box box-info'><div class='box-header'><h3 class='box-title'>".$lang['db_create']."</h3> ".helpLink($lang['help2'])."</div>"; 
	echo "<div class='box-body'>";
	echo "<form name='create_database' method='post' action='".PAGE."'>";
	echo $token_html;
	echo "<div class='input-group'>";
		echo '<span class="input-group-addon"><span class="glyphicon glyphicon-hdd" style="color: black;"></span></span>';
	echo "<input type='text' class='form-control' name='new_dbname' placeholder='Database name'/>";

	echo '</div>';
	if(class_exists('SQLiteDatabase') && (class_exists('SQLite3') || class_exists('PDO')))
	{
		echo "<select name='new_dbtype' class='selection newDbType'>";
		echo "<option value='3'>SQLite 3</option>";
		echo "<option value='2'>SQLite 2</option>";
		echo "</select>";
	}
	echo "<button type='submit'  class='btn'>".$lang['create']."</button>";
	echo "</form>";
	echo "</div>";
	echo "</div>";
	echo "</div></div>";
...
echo "<form action='".PAGE."' method='post'>";
echo $token_html;
echo "<button type='submit'  name='logout' class='btn btn-sm btn-primary'>".$lang['logout']."</button>";
echo "</form>";
echo "</div>";
echo "<strong>".$lang['powered']." <a href='".PROJECT_URL."' target='_blank' style='font-size:11px;'>".PROJECT."</a> | ";
