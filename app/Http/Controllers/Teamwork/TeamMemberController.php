...
class TeamMemberController extends Controller
{
    ...
    public function invite(Request $request, $team_id)
    {
        ...
        if( !Teamwork::hasPendingInvite( $request->name, $team) )
        {
            Teamwork::inviteToTeam( $request->name, $team, function( $invite )
            {
                   $message =  "You have been invited to join team ".$team->name."<br>
Click here to join: <a href='".secure_url('/legions/accept', $invite->accept_token)."'>".secure_url('/legions/accept', $invite->accept_token)."</a>";
                  $subject = "You are invited to be a part of the legion named \"".$team->name."\"";
                  $recipients = User::where("name",$request->name)->get()[0]->id;
                  $client = new Client();
                  $res = $client->request('POST','/message/craft',  
                 [ 'form_params' => [
        'message' => $message,
        'subject' => $subject,
